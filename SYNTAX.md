...
| 参数 | 类型 | 默认值 | 说明 |
|------|------|--------|------|
| `color` | `string` / `[string, ...]` / `random` / `none` | `random` | 主颜色。`random` 随机选取颜色；`"#ff0043"` 固定；`["#ff0","#0f0"]` 多色；`none` 星点隐形（仅显示火花/流束） |
...
```
flower 数量 (花瓣数) { color = 颜色, life = 600, speed = 速度 }
```
...
子粒子横向分散并受重力下落，默认重力 `1.6`（可在选项中用 `gravity` 覆盖）。参数：`数量`（1 – 200）。
